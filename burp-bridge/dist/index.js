...
async function ensureEventSource() {
    if (typeof globalThis.EventSource === "undefined") {
        ...
        globalThis.EventSource = mod.EventSource ?? mod.default;
    }
}
let client = null;
...
function withTimeout(p, ms, label) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
            const err = new Error(`${label} timed out after ${ms}ms (SSE likely stalled)`);
...
app.get("/tools", async (_req, res) => {
    try {
        ...
        if (!toolsCache.length) {
            const r = await withTimeout(c.listTools(), CALL_TIMEOUT_MS, "listTools");
            toolsCache = r?.tools ?? [];
        }
        res.json({
            ok: true,
...
app.post("/call", async (req, res) => {
    ...
    try {
        const c = await connect();
        const result = await withTimeout(c.callTool({ name, arguments: args }), CALL_TIMEOUT_MS, `callTool(${name})`);
...
async function runHeartbeat() {
    ...
    try {
        const c = await connect();
        const toolNames = toolsCache.map((t) => t.name);
        const useTool = toolNames.includes(HEARTBEAT_TOOL) ? HEARTBEAT_TOOL : null;
        if (useTool) {
            await withTimeout(c.callTool({ name: useTool, arguments: { text: "gc-heartbeat" } }), HEARTBEAT_TIMEOUT_MS, `heartbeat(${useTool})`);
        }
        else {
...
process.on("uncaughtException", (err) => console.error(`[BurpBridge] uncaughtException (continuing): ${err.message}`));
process.on("unhandledRejection", (reason) => console.error(`[BurpBridge] unhandledRejection (continuing): ${reason}`));
...
process.on("SIGTERM", () => console.error("[BurpBridge] SIGTERM received — ignoring to keep Burp SSE alive."));
process.on("SIGINT", () => console.error("[BurpBridge] SIGINT received — ignoring to keep Burp SSE alive."));
// SIGHUP fires when the parent shell (bash -c wrapper) exits in non-interactive mode.
process.on("SIGHUP", () => console.error("[BurpBridge] SIGHUP received — ignoring to keep Burp SSE alive."));
// Log exit cause so users can diagnose unexpected bridge deaths.
process.on("exit", (code) => console.error(`[BurpBridge] Exiting with code ${code} — if unexpected, check for SIGKILL or fatal error above.`));
